"""
Training script for hand gesture recognition model with NO_GESTURE class.
...
and inference pipeline, including a NO_GESTURE class to properly handle
idle/random hand poses.
"""
...
TRAINED_MODEL_PATH = 'output/trained_models/handtalk_model_with_nogesture.keras'
BEST_MODEL_PATH = 'output/trained_models/best_model_with_nogesture.keras'
LABEL_ENCODER_PATH = 'output/model_assets/label_encoder_with_nogesture.pkl'
LABELS_TXT_PATH = 'output/model_assets/labels_with_nogesture.txt'
...
def load_and_validate_data(data_path='../../datasets/gestures_bimanual_expanded.csv'):
    """
    ...
    """
    print("Loading expanded dataset with NO_GESTURE class...")
    
    # Check if data file exists
    if not os.path.exists(data_path):
        # Try alternative path
        alt_path = 'datasets/gestures_bimanual_expanded.csv'
...
def build_standard_model(input_dim, num_classes):
    ...
    ✅ Includes NO_GESTURE class in output
    """
    model = Sequential([
        # Input layer
        ...
        # Output layer - now includes NO_GESTURE class
    ...
    print(f"Output layer has {num_classes} classes (includes NO_GESTURE)")
    return model
...
def main():
    ...
    """
    print("="*70)
    print("HAND GESTURE RECOGNITION MODEL TRAINING PIPELINE WITH NO_GESTURE CLASS")
    print("="*70)
    ...
    try:
        ...
        # Verify NO_GESTURE class is present
        if 'NO_GESTURE' not in label_encoder.classes_:
            print("⚠️  Warning: NO_GESTURE class not found in dataset!")
        else:
            print("✅ NO_GESTURE class successfully included in training")
        ...
        print("  - output/trained_models/handtalk_model_with_nogesture.keras (trained Keras model)")
        print("  - output/model_assets/label_encoder_with_nogesture.pkl (label encoder)")
        print("  - output/model_assets/labels_with_nogesture.txt (text labels for Android)")
        print("  - output/trained_models/best_model_with_nogesture.keras (best performing checkpoint)")
